...
negative.binomial <- function (theta = stop("'theta' must be specified"),
                               link = "log")
{
  ...
  if (linktemp %in% c("log", "identity", "sqrt"))
    ...
    if (inherits(link, "link-glm")) {
      ...
      gettextf(paste("\"%s\" link not available for negative binomial family;",
                     "available links are \"identity\", \"log\" and \"sqrt\"",
                     linktemp))
    )
  }
  .Theta <- theta
  ...
  dev.resids <- function(y, mu, wt) 2 * wt *
    (y * log(pmax(1, y)/mu) - (y + .Theta) * log((y + .Theta)/(mu + .Theta)))
  ...
  environment(variance) <- environment(validmu) <- environment(dev.resids) <-
    environment(aic) <- environment(simfun) <- env
