...
To illustrate elastic net regression with linear models, we will use the [Prostate Cancer Data](https://www.rdocumentation.org/packages/ElemStatLearn/versions/2015.6.26/topics/prostate) from the `ElemStatLearn` package. These data come from a study by Stamey et al. (1989) of 97 men. Measurements include the log of prostate specific antigen (PSA, `lpsa`), log of cancer volume (`lcavol`), log of prostate weight (`lweight`), `age`, log of benign prostatic hyperplasia amount (`lbph`), seminal vesicle invasion (`svi`), log of capsular penetration (`lcp`), Gleason score (`gleason`), and percent of Gleason scores 4 or 5 (`pgg45`).).
...
The prostate data includes a suggested train/test split variable, `train`. Let's use this to construct a `data_partition` object. Like `beset_lm` and `beset_glm`, `beset_elnet` can work with `data_partition` objects directly: it will tune alpha and lambda by cross-validating the training data, and use the test data to assess performance of the model. 
...
In this example, the minima is at $\alpha = 0.01$ and $\lambda = 0.075$, which is a predominantly ridge regression favoring the inclusion of all 8 predictors, which are ranked in the coefficient table by order of the absolute value of standardized coefficient. Note that the cross-validated $R^2$ (0.6), while not as overly optimistic as the train-sample $R^2$ (0.69), overestimates prediction performance as compared to the independent test sample ($R^2 = 0.53$). The bracketed numbers after the cross-validated $R^2$ give the range of $R^2$ values that were observed over the 10 repetitions of cross-validation and thus gauge the sensitivity of the cross-validation average to the specific randomized fold assignments.  

The horizontal line in the plot corresponds to the upper error limit of this minima. As a rule-of-thumb (called the "1SE rule"), points on the curve that fall beneath this line indicate performance that is practically equivalent to the very best. In practice, one can often increase the generalizability of a model by picking a greater penalty than the one that achieves the absolute minimum cross-validation error. Note that there are `lambda` values at every value of `alpha` that fall beneath this line, and that with greater sparsity (larger values of `alpha`), smaller values of `lambda` are needed to optimize prediction. By default, the `summary` method will first pick the largest `alpha` that has part of its tuning curve within 1SE of the minimum cross-validation error, and then the largest value of `lambda` within the 1SE limit for that value of `alpha`:

```{r}
summary(mod)
```

Notice that this leads to a much sparser model with only 3 predictors, and the gap between the $R^2$ estimated for the test sample vs. the tune sample has been eradicated by using the 1SE rule.
...
You may have noticed something odd when comparing the graphs to the summary output. The graphs seem to suggest that performance on the single test holdout is actually *better* than what the cross-validation error would predict; for that matter, there are parts of the curve where it is even better than what the *training* error would predict. Yet, the summary output shows that the `Test` $R^2$ is either the same or worse than the `CV` $R^2$.

The reason for this points out the limitations of using a single `Test` set, especially with small sample sizes such as this one. It so happens that the range of values for the response is much greater for the training set that it is for the test set:
...
Relatedly there is more variance in the training set than in the test set:
...
So the fact that `Test` MSE is lower than `CV` MSE simply reflects that the `Test` set tends to have smaller residuals because it is less variable overall. You don't see this with the $R^2$ statistic because it, by definition, normalizes the MSE by the variance. So in this case a plot of $R^2$ may provide a more valid comparison:
...
As you can see from the above example, a single test set may not provide a repesentative example of general test error, but also cross-validation estimates of test error may be biased they are used to select model parameters. One solution is to perform a nested cross-validation, i.e., have an outer cross-validation for estimating test error and an inner cross-validation (nested within the training folds of the outer cross-validation) for tuning. This can be accomplished by using the argument `nest_cv` = TRUE, but be warned: this can be computationally expensive.

To make things more interesting, let's add an equal number of variables that we know to be noise to see if the elastic net is able to effectively deal with these.
...
Observe where the red curve intersects with the horizontal dashed line and how this reliably captures the approximate point at which the CV holdout and Test holdout curves converge. Thus, one can be reasonably confident that the if the parameters are selected with the 1SE rule, the associated error is not optimistically biased by the selection. And even when selecting the based on the optimal tuning parameters, the bias is not large.
...
On the one hand, this has a bad result in that some of the noise predictors make it into the model, two of them (`noise2` and `noise4`) reliably so. (The others are shrunk to 0 on at least some cross-validation runs, which is indicated by a `0.000` value in either the `Min` or `Max` column.) On the other hand, the drop-off in variance explained between the CV and Test holdouts is minimal (about 1% point), and the performance on the Test holdouts is slightly better than what was obtained under the 1SE rule. 
