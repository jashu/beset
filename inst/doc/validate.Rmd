...
For example, consider this example model that uses the [Prostate Cancer Data](https://www.rdocumentation.org/packages/ElemStatLearn/versions/2015.6.26/topics/prostate) from the `ElemStatLearn` package. These data come from a study by Stamey et al. (1989) of 97 men. Measurements include the log of prostate specific antigen (PSA, `lpsa`), log of cancer volume (`lcavol`), log of prostate weight (`lweight`), `age`, log of benign prostatic hyperplasia amount (`lbph`), seminal vesicle invasion (`svi`), log of capsular penetration (`lcp`), Gleason score (`gleason`), and percent of Gleason scores 4 or 5 (`pgg45`).

Suppose we have a hypothesis that Gleason score predicts PSA. Since there are only 4 unique Gleason scores (6, 7, 8, and 9), let's first convert this from a numeric to a factor variable.
...
Further, since there are so few observations of Gleason scores greater than 7, let's convert this to a binomial variable, with levels of `6` and `> 6`.
...
Finally, we fit our hypothesized linear model.
...
1. **Mean absolute error.** This corresponds to the mean of the absolute value of the prediction residuals. On average, our model predictions tend to miss the true PSA score by 0.765 log units. 
...
In this case, we can see that our model reduces the mean cross entropy from 1.56 to 1.44, a 7.7% improvement.

3. **Mean squared error.** The traditional performance metric for regression models of a continuous response.  

4. **R-squared.** It is well known that, unless the sample size $n$ greatly exceeds the number of predictors $p$, a model's $R^2$ will be inflated. The adjusted $R^2$ that appears in the `summary` output of an `lm` is an attempt to correct for this bias by penalizing the $R^2$ in proportion to larger $p$ and smaller $n$. The cross-validated $R^2$ can also be thought of as an adjusted $R^2$, except it is empirically, rather than formulaically, derived. It is the fraction of variance that the model is expected to *predict* in a *new* sample. At least in the case of the `prostate` data set, the cross-validated $R^2$ is somewhat more pessimistic than the adjusted $R^2$ statistic.
...
If you `validate` a binomial GLM, "Mean Absolute Error" will be replaced by "Area Under Curve", i.e., the area under the receiver operating characteristic (ROC) curve, and "Variance Explained" will be replaced by "Deviance Explained". For example, we can flip our previous model and predict the Gleason score category as a function of PSA scores:
...
To perform leave-one-out cross-validation (LOOCV), set `n_folds` to a value equal to the number of observations in your data set. (Technically, any value greater than half the number of observations will also result in LOOCV because `validate` will autocorrect any such value to equal the number of observations.) Note that under LOOCV repetitions are pointless and will not be performed, and so ranges will not be reported in the output.

```{r}
validate(log_mod, n_folds = nobs(log_mod), n_reps = 1)
```

Note also that for the AUC statistics and variance/deviance explained, error metrics cannot be calculated under LOOCV because these statistics are undefined for individual predictions; they can only be calculated on the aggregate predictions. 
