...
output: 
  rmarkdown::html_vignette:
    css: 
      ...
      - style.css
vignette: >
  %\VignetteIndexEntry{`beset`: Best-subset selection of GLMs}
...
3. By default, `beset_lm` and `beset_glm` will search for models with up to 10 predictors (`p_max = 10`). If you need to fit a larger number of parameters, or if the number of relevant parameters exceeds 1/10 of the sample size, it is recommended that you switch to the `beset_elnet` function instead. And `beset_lm` and `beset_glm` will not allow the number of model parameters to exceed 1/3 of the sample size of the training data.
